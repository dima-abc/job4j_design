...
BEGIN TRANSACTION;
/*Объявляем курсор*/
DECLARE curs_prior SCROLL CURSOR FOR SELECT * FROM products;
/*перемещаемся в конец курсора*/
MOVE FORWARD 20 FROM curs_prior;
/*проверяем что вперед элементов нет*/
FETCH NEXT FROM curs_prior;
/*Получаем предыдущие элементы*/
FETCH PRIOR FROM curs_prior;
FETCH PRIOR FROM curs_prior;
/*Получаем 10 предыдущих строк*/
FETCH BACKWARD 10 FROM curs_prior;
/*Получаем предыдущие элементы*/
FETCH PRIOR FROM curs_prior;
FETCH PRIOR FROM curs_prior;
/*Закрываем курсор*/
